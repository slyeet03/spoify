...
#[tokio::main]
pub async fn search_selected_artist_tracks(app: &mut App, search: &mut Search) -> Result<(), ClientError> {
...
pub fn process_selected_artist_tracks(app: &mut App, search: &mut Search) {
    // Clear any existing user track data in the app before processing
    search.selected_artist_tracks_names.clear();
    search.selected_artist_track_album_names.clear();
    search.selected_artist_tracks_duration.clear();
    ...
    for track_data in json_data {
        if let Value::Object(track_obj) = track_data {
            if let Some(track_name) = track_obj.get("name").and_then(Value::as_str) {
                search.selected_artist_tracks_names
            ...
            if let Some(external_urls) = track_obj.get("external_urls").and_then(Value::as_object) {
                if let Some(track_link) = external_urls.get("spotify").and_then(Value::as_str) {
                    search.selected_artist_tracks_links
